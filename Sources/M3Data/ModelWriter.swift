...
public class ModelWriter {
    ...
    public func generateFileWrappers() throws -> (data: FileWrapper, content: FileWrapper) {
        ...
        for supportedType in self.plist.supportedTypes {
            let (plistRepresentations, modelContents) = try self.generateData(for: supportedType.modelType)
    ...
    private func generateData(for modelType: ModelType) throws  -> ([ModelObjectPlistRepresentation], [ModelFile]) {
        var plistItems = [ModelObjectPlistRepresentation]()
        var files = [ModelFile]()
        ...
        try sortedItems.forEach { (object) in
			let (updatedPlist, modelFiles) = try object.plistRepresentation.convertModelFiles(from: object.modelFileProperties)
			plistItems.append(updatedPlist)
			files.append(contentsOf: modelFiles)
        }
